...
from openapi_llm.utils import normalize_function_name
...
def create_operation_id(path: str, http_method: str = "get") -> str:
    """
    Creates a standardized operationId from a path and HTTP method.

    Generates a synthetic operationId when one is not provided in the OpenAPI spec
    by combining the path and HTTP method.
...
class Operation:
    ...
    def get_parameters(
        self, parameter_location: Optional[Literal["header", "query", "path"]] = None
    ) -> List[Dict[str, Any]]:
        """
        Get operation parameters filtered by location.

        Retrieves parameters from the operation, optionally filtered by their location
        in the request (header, query, or path).

        :param parameter_location: The location of the parameters to get.
        :returns: The parameters for the operation as a list of dictionaries.
        """
        if parameter_location:
            return [
                param for param in self.parameters if param["in"] == parameter_location
            ]
        return self.parameters

    def get_server(self, server_index: int = 0) -> str:
        """
        Get the server URL for this operation.

        Retrieves the server URL from operation, path, or root level, in that order of precedence.
    ...
    def find_operation_by_id(self, op_id: str) -> Operation:
        ...
        for path, path_value in self.spec_dict.get("paths", {}).items():
            ...
            for method, operation_dict in operations.items():
                operation_id = operation_dict.get(
                    "operationId", create_operation_id(path, method)
                )
